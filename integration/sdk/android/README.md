...
以下接口生效依赖于可视化热图模块，需集成可视化热图SDK相关analysys_visual_xxx.jar文件，请正确集成。
...
加密模块生效依赖于加密SDK模块，需集成加密SDK相关analysys_encrypt_xxx.jar文件，请正确集成。

在初始化SDK时需设置加密方式，目前只支持 AES 加密，AES加密分为EncryptEnum.AES（128位密钥，ECB加密模式）和 EncryptEnum.AES_CBC（128位密钥，CBC加密模式），如不设置此参数，数据上传不加密。
