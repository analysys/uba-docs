...
|                 js文件                |          功能描述         | 是否必须 |
| :---------------------------------: | :-------------------: | :--: |
|     AnalysysAgent_QQ_SDK.min.js     |        基础模块SDK        |  二选一 |
|   AnalysysAgent_QQ_SDK.es6.min.js   |      基础模块ES6语法SDK     |  二选一 |
...
* _autoTrack_ 设置是否开启全埋点,false - 不开启全埋点(默认)；true - 开启全埋点；开启全埋点将会上报所有绑定（支持tab、longtab、longpress）事件,并上报$user_click 事件,设置data-content为采集的 $element_content、data-type为采集的 $element_type、data-name为采集的$element_name、id为采集的$element_id。不设置采集不到。不支持系统方法包括生命周期事件的上报，如果要采集tabbar切换，务必在注册Page的时候注册OnTabItemTap方法，否则采集不到。
...
![](<../../../.gitbook/assets/qqRequest (1).png>)
